...
export default function PedidosPage() {
  ...
  useEffect(() => {
    const enviados: any[] = JSON.parse(localStorage.getItem("pedidos_enviados") || "[]");
    const config: any[] = JSON.parse(localStorage.getItem("mesas_pos") || JSON.stringify(mesasBase));
    const actualizadas: Mesa[] = config.map((mesa: any) => {
      const n: string = mesa.nombre;
      ...
      if (cobrar) return { ...mesa, estado: "Por cobrar" as EstadoMesa, total: cobrar.total };
      if (activo) return { ...mesa, estado: "Ocupada" as EstadoMesa, total: activo.total };
      return { ...mesa, estado: "Libre" as EstadoMesa, total: undefined };
    });
