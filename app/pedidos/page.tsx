...
export default function PedidosPage() {
  ...
  useEffect(() => {
    ...
    const actualizadas = config.map((mesa) => {
      const n = mesa.nombre;
      const activo = enviados.find((p) => p.mesa === n && (p.estado === 'Enviado' || p.estado === 'En preparación'));
  ...
  return (
    <main className='min-h-screen bg-zinc-950 text-white p-6'>
      ...
      <section className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
        {mesas.map((mesa) => (
          <a key={mesa.id} href={'/pedidos/' + mesa.nombre.toLowerCase().replace(' ', '-')} className={'rounded-2xl border bg-zinc-900 p-6 transition ' + border(mesa.estado)}>
